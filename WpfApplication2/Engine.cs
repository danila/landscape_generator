...
namespace WpfApplication2
{
    ...
    public class Engine
    {
        ...
        public Engine(WriteableBitmap bmp)
        {
            this.bmp = bmp;
        ...
        // during each frame
        public void Render(Camera camera, Vector3 lightPos, params Terra[] meshes)
        {
            ...
            foreach (Terra mesh in meshes)
            {
                ...
                foreach (var triangle in mesh.Polygons)
                {
                    ...
                    double water = mesh.WaterFactor;

                    if (triangle.A.Coordinates.Y <= maxY * (water - 0.1))
                        terrain = deepsea;
                    else if (triangle.A.Coordinates.Y < maxY * water)
                        terrain = SharpDX.Color.RoyalBlue;
                    else if (triangle.A.Coordinates.Y < maxY * (water + 0.07))
                        terrain = desert;
                    else if (triangle.A.Coordinates.Y < maxY * (water + 0.3))
                        terrain = grassland;
                    else if (triangle.A.Coordinates.Y < maxY * (water + 0.6))
                        terrain = decidousforest;
                    else if (triangle.A.Coordinates.Y < maxY* (water + 0.8))
                        terrain = bare;
